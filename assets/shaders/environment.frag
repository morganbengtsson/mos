...
void main() {
    ...
    for(int i = 0; i < 2; i++) {
      ...
      vec3 radiance = pow(light.strength * 0.05, 2) * light.color * attenuation;
    ...
    vec3 ambient = (albedo * lights[0].color * pow(lights[0].strength, 0.25)/40.0
    + albedo * lights[1].color * pow(lights[1].strength, 0.25)/40.0);
